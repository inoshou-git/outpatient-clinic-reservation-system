...
export const loginUser = async (userId: string, password?: string): Promise<{ token: string; user: Partial<User>; mustChangePassword?: boolean } | null> => {
    const db = await readDb();
    const user = db.users.find(u => u.userId === userId && u.password === password && !u.isDeleted);
    if (user) {
...
export const setUserPassword = async (userId: string, newPassword?: string): Promise<boolean> => {
    const db = await readDb();
...
export const getAllUsers = async (): Promise<Partial<User>[]> => {
    const db = await readDb();
    return db.users.filter(u => !u.isDeleted).map(({ password, ...user }) => user);
};

export const createUser = async (userData: any): Promise<{ user?: Partial<User>, error?: string, status?: number }> => {
...
export const updateUser = async (userId: string, userData: any): Promise<Partial<User> | null> => {
    const db = await readDb();
    const userIndex = db.users.findIndex(u => u.userId === userId);
    if (userIndex !== -1) {
...
export const deleteUser = async (userId: string, lastUpdatedBy: string): Promise<boolean> => {
    const db = await readDb();
    const userIndex = db.users.findIndex(u => u.userId === userId);
    if (userIndex !== -1) {
