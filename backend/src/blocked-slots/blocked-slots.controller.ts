...
export const createBlockedSlot = async (req: Request, res: Response) => {
  if ((req as any).user.role === "viewer") {
    return res
      .status(403)
      .json({ message: "閲覧ユーザーは予約不可設定を作成できません。" });
  }
...
export const updateBlockedSlot = async (req: Request, res: Response) => {
  if ((req as any).user.role === "viewer") {
    return res
      .status(403)
      .json({ message: "閲覧ユーザーは予約不可設定を更新できません。" });
  }
  const id = parseInt(req.params.id, 10);
  const result = await blockedSlotService.updateBlockedSlot(
    id,
    req.body,
    (req as any).user.name
  );
...
export const deleteBlockedSlot = async (req: Request, res: Response) => {
  if ((req as any).user.role === "viewer") {
    return res
      .status(403)
      .json({ message: "閲覧ユーザーは予約不可設定を削除できません。" });
  }
  const id = parseInt(req.params.id, 10);
  const success = await blockedSlotService.deleteBlockedSlot(
    id,
    (req as any).user.name
  );
  if (success) {
...
export const registerHolidays = async (req: Request, res: Response) => {
  ...
  try {
    const result = await blockedSlotService.registerHolidays(
      (req as any).user.name
    );
