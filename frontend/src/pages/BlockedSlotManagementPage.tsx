...
const BlockedSlotManagementPage = () => {
  ...
  const { isBlockedSlotFormOpen, openBlockedSlotForm, closeBlockedSlotForm } =
    useUI();
  ...
  const handleBulkDelete = async () => {
    if (
      window.confirm(
        `${selectedBlockedSlots.length}件の予約不可設定を削除してもよろしいですか？`
      )
    ) {
      try {
  ...
  const filteredBlockedSlots = useMemo(() => {
    return blockedSlots.filter((slot) =>
      dayjs(slot.date).isSame(currentDate, "month")
    );
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
              ...
              {filteredBlockedSlots.map((slot) => {
                ...
                return (
                  <TableRow
                    ...
                    hover
                    onClick={(event) =>
                      bulkActionEnabled && handleSelectClick(event, slot.id)
                    }
                    role="checkbox"
      ...
      <Dialog open={isBlockedSlotFormOpen} onClose={closeBlockedSlotForm}>
        <DialogTitle>
          {editingBlockedSlot ? "予約不可設定の編集" : "新規予約不可設定の登録"}
        </DialogTitle>
        <DialogContent>
          <BlockedSlotForm
